...
#include "scheme.hpp"
// Included again because some of the defines are undefined at the end of
// scheme.hpp
#include "error.hpp"
    ...
    while (point < end) {
        ...
        if (info.type != moved_cell_type) {
            Destroy_Function destructor =
                ip_->type_manager.Get_Destructor(info.type);
            if (destructor != NULL)
                destructor(current);
        }
        point += info.size;
...
const size_t default_workspace_size = 503, null_env_size = 23,
             report_env_size = 233;
...
Cell Run_Code(Cell code, bool handle_errors) {
    ...
    while (!done) {
        try {
            ...
            if (handle_errors &&
                Get_Value(ip_->report_env, handle_error) != invalid_cell) {
...
Output_Catcher::Output_Catcher() : _old_stream(ip_->output) {
    ip_->output = Make_Outport(_stream);
}

Output_Catcher::~Output_Catcher() { ip_->output = _old_stream; }
...
#define CHECK_TYPE(var, type, num)                                             \
...
Cell p_current_input_port() { return ip_->input; }
Cell p_current_output_port() { return ip_->output; }
...
Cell p_current_time() { return Make_Integer(time(NULL)); }
